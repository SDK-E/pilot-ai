...
function parseSearchExtraction(value: unknown): StagehandSearchResult[] {
  ...
  const results: StagehandSearchResult[] = [];
  ...
  for (const item of candidate) {
    ...
    const snippet =
      typeof record.snippet === "string" ? record.snippet.trim() : undefined;
...
export async function performStagehandSearch(
  ...
  return parseSearchExtraction(extracted.data);
}
      ...
      switch (action) {
        ...
        case "invokeWebMCPTool": {
          ...
          if (!isJsonRecord(input)) {
            throw new Error(
              "toolInput must be a flat JSON object for invokeWebMCPTool.",
            );
          }
          ...
          const tool = tools.find(
            (candidate) =>
              ...
              (!frameId || candidate.frameId === frameId),
          );

          if (!tool) {
            throw new Error(
              `WebMCP tool "${toolName}" was not found on the current page.`,
            );
          }
