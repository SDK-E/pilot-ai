...
Tool failures are recoverable unless evidence shows otherwise.
...
2. determine whether the failure is:
   ...
   - missing data
   - tool-specific
3. do not abandon the user's objective merely because one tool failed
...
- try another query
- use another source
- switch from HTTP fetch to browser
- switch from browser to search
- use site discovery
- use structured data
...
- continue another useful research branch when possible
- retry later if still needed

SOURCE FAILURE

If one source is inaccessible:
- look for the canonical page
...
- preserve the blocker in working memory
...
When a meaningful research path becomes blocked:
