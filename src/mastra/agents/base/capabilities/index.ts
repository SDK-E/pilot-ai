...
import { createConnectorTool } from "../../../tools/connectors/connector.js";
...
export const CAPABILITIES: Record<CapabilityId, Capability> = {
  ...
  connector: {
    ...
    toolNames: ["connector"],
    instructions:
      "Use connector to call this organization's connected connectors (GitHub, Slack, and any admin-added ones — all read-only). Call it with action \"list-connectors\" first to discover what's available before calling a specific one.",
    tools: ({ command, runtimeToken }) => ({
      connector: createConnectorTool({ command, runtimeToken }),
    }),
  },
};
