...
import {
  ...
  searchResultSchema,
  type SearchResult,
} from "../tools/search/langsearch.js";
import { performUrlFetch } from "../tools/web/url-fetch.js";
...
const technicalResearchStep = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    const sources = dedupe([...enriched, ...discovered.slice(14)]);

    const agent = mastra?.getAgent("developmentAgent");

    if (!agent) {
      throw new Error("developmentAgent not found");
    }
...
export const technicalReviewWorkflow = createWorkflow({
  id: "technical-research",
  inputSchema,
  outputSchema,
})
