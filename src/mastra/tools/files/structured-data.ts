...
function flattenJsonLd(value: unknown): unknown[] {
  ...
  if (
    ...
    return (value as { "@graph": unknown[] })["@graph"].flatMap(flattenJsonLd);
  }
...
export const structuredData = createTool({
  ...
  execute: async ({ url }, { abortSignal }): Promise<StructuredDataOutput> => {
  ...
  toModelOutput: (output) => ({
    type: "text",
    value: JSON.stringify(output.items, null, 2).slice(0, 60_000),
  }),
});
