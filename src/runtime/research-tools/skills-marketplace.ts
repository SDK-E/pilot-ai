...
const CURATED_TTL_MS = 60 * 60_000;
...
const rankedSkillSchema = skillSummarySchema.extend({
  official: z.boolean(),
  score: z.number(),
...
type RankedSkill = z.infer<typeof rankedSkillSchema>;

interface SkillsSearchResponse {
  data?: unknown;
}

interface SkillsCuratedResponse {
  data?: unknown;
}
...
async function skillsRequest<T>(
  ...
  const response = await fetch(`${SKILLS_API}${path}`, {
    signal: abortSignal,
    headers: {
      ...
      "x-agent-purpose": "runtime-skill-discovery",
    },
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(`skills.sh ${response.status}: ${body}`);
  }
...
async function cachedSkillsRequest<T>(
  ...
  const key = makeCacheKey(cacheType, cacheInput);
  ...
  const value = await skillsRequest<T>(path, abortSignal);

  await setCachedValue(key, cacheType, value, ttlMs);

  return value;
}

function parseSearchResults(value: unknown): SkillSummary[] {
  const parsed = z.array(skillSummarySchema).safeParse(value);
...
function parseCuratedIds(value: unknown): Set<string> {
  ...
  for (const entry of value) {
    ...
    for (const skill of skills) {
      const parsed = skillSummarySchema.safeParse(skill);
...
function emptyFeedback(skillId: string): SkillFeedbackStats {
  return {
    skillId,
    uses: 0,
    helpful: 0,
...
async function rankSkills(
  ...
  const maxInstalls = Math.max(...skills.map((skill) => skill.installs), 1);

  const feedback = await getSkillFeedbackMap(skills.map((skill) => skill.id));

  return skills
    .map((skill, index) => {
      const relevance = 1 - index / Math.max(skills.length, 1);

      const popularity = Math.log1p(skill.installs) / Math.log1p(maxInstalls);

      const isOfficial = officialIds.has(skill.id);

      const history = feedback.get(skill.id) ?? emptyFeedback(skill.id);
      ...
      return {
        ...
        learnedScore: Number(history.learnedScore.toFixed(4)),
        uses: history.uses,
        helpful: history.helpful,
        unhelpful: history.unhelpful,
      };
    })
...
function parseLoadedFiles(value: unknown): {
  loadedFiles: {
    path: string;
    contents: string;
  }[];
...
function parseAudits(value: unknown) {
  const parsed = z.array(auditSchema).safeParse(value);

  return parsed.success ? parsed.data : [];
}

function unsafeAudit(audits: z.infer<typeof auditSchema>[]): boolean {
  return audits.some((audit) => {
    const status = audit.status.toLowerCase();

    const risk = audit.riskLevel?.toLowerCase();
...
async function getCuratedIds(abortSignal?: AbortSignal): Promise<Set<string>> {
  try {
    const response = await cachedSkillsRequest<SkillsCuratedResponse>(
      "skills-curated",
      "official",
      "/skills/curated",
      CURATED_TTL_MS,
    ...
    return parseCuratedIds(response.data);
  } catch {
    return new Set();
  }
}

export const skillsMarketplace = createTool({
  ...
  outputSchema: z.union([
    z.object({
      action: z.literal("search"),
      skills: z.array(rankedSkillSchema),
    }),
    ...
    z.object({
      ...
      loadedFiles: z.array(loadedFileSchema),
  ...
  execute: async (input, { abortSignal }) => {
    if (input.action === "feedback") {
      const stats = await recordSkillFeedback(
        input.id,
        input.helpful,
        input.query,
        input.reason,
    ...
    if (input.action === "search") {
      ...
      const candidateLimit = Math.min(Math.max(input.limit * 3, 10), 50);
      ...
      const [response, officialIds] = await Promise.all([
        ...
        getCuratedIds(abortSignal),
      ]);

      const skills = (
        await rankSkills(
          parseSearchResults(response.data).filter(
            (skill) => !skill.isDuplicate,
          ),
          officialIds,
        )
      ).slice(0, input.limit);
    ...
    const encodedId = input.id.split("/").map(encodeURIComponent).join("/");

    const [detail, auditResponse, officialIds] = await Promise.all([
    ...
    const audits = parseAudits(auditResponse.audits);
    ...
    const { loadedFiles, skippedFiles } = parseLoadedFiles(detail.files);

    if (loadedFiles.every((file) => file.path.toLowerCase() !== "skill.md")) {
      throw new Error(
        `Skill ${input.id} does not expose a readable SKILL.md file.`,
      );
    }

    const id = typeof detail.id === "string" ? detail.id : input.id;

    await recordSkillUse(id, input.query);
  ...
  toModelOutput: (output) => {
    if (output.action === "feedback") {
      return {
        type: "text",
        value: `Recorded skill feedback for ${output.stats.skillId}. Learned score: ${output.stats.learnedScore.toFixed(3)}.`,
      };
    }
