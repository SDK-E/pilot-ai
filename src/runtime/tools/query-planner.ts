...
function buildStrategies(
  ...
  const raw = [
    ...
    },
    {
      ...
      rationale:
        "Search for explicit evidence or intent signals instead of topic similarity.",
      score: 0.95,
    },
    {
      kind: "primary" as const,
      query: [base, primarySite ? `site:${primarySite}` : "official"]
        .filter(Boolean)
        .join(" "),
    ...
    },
    {
      ...
      query:
        signals.length > 0
          ...
          : `"${base}"`,
      rationale:
        "Use a narrow exact-signal query without over-constraining every dimension at once.",
      score: 0.85,
    },
    {
      ...
      query: [base, year ? `after:${Number(year) - 1}-01-01` : undefined]
        ...
        .join(" "),
      rationale:
        "Prefer recent evidence when the objective is current or time-sensitive.",
      score: 0.8,
    },
  ];
...
function adapt(
  objective: string,
  attempts: z.infer<typeof attemptSchema>[],
  signals: string[],
  sites: string[],
  currentDate?: string,
) {
  ...
  const successes = attempts.filter(
    (item) => item.usefulCount > 0 || item.resultCount > 2,
  );

  const base = buildStrategies(objective, signals, sites, currentDate).filter(
    (item) => !attempted.has(item.query),
  );
  ...
  const best = [...successes].sort(
    (a, b) =>
      b.usefulCount * 3 + b.resultCount - (a.usefulCount * 3 + a.resultCount),
  )[0];
