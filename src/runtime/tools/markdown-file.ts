...
function sanitizeFilename(value: string): string {
  ...
  return /\.md(?:own)?$/i.test(sanitized) ? sanitized : `${sanitized}.md`;
}
...
export const markdownFile = createTool({
  ...
  inputSchema: z.object({
    filename: z.string().min(1).default("pilot-export.md"),
  ...
  toModelOutput: (output) => ({
    type: "text",
    value: `Created ${output.filename} with ${output.characterCount} characters.`,
  }),
});
