...
- **File mode** (default): give a path (relative or absolute, `.ts`/`.mts`). Reports what that file imports and what imports it, expanding outward `--depth` hops (default 1).
...
- Start with `--depth 1`; only increase it if the immediate neighbors aren't enough context — depth grows the result fast in a well-connected file, especially around `src/mastra/server/`.

## When to reach for this first

Before editing a shared type, a widely-imported util, or a Mastra tool/agent definition, run this to see the actual importer list rather than assuming from the file's location. It's meant to replace "grep + open five files to check" with one command.
