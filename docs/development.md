...
| `LANGSEARCH_API_KEY`                                    | To use `web-search`                           | LangSearch API key, only as a local-dev fallback — in a real deployment Pilot forwards its admin-managed value as the `x-pilot-langsearch-api-key` header on every request instead.                                                                                                          |
| `GITHUB_TOKEN`                                          | Optional                                      | Raises the unauthenticated rate limit for the public-web GitHub tool, only as a local-dev fallback — Pilot forwards its admin-managed value as `x-pilot-github-token` instead. Never required either way.                                                                                    |
