...
| `EDGE_CONFIG`                                           | To use `web-search`/`code-sandbox`/connectors | Vercel Edge Config connection string backing the three platform circuit breakers (`webSearchEnabled`, `codeSandboxEnabled`, `connectorsEnabled` — see `src/mastra/server/feature-flags.ts`), independent of any org's own preference. Toggle from the Edge Config store in the Vercel dashboard, no redeploy needed. Missing/unreachable fails closed (all disabled). |
...
| `GITHUB_TOKEN`                                          | Optional                                      | Raises the unauthenticated rate limit for the public-web GitHub tool. Never required.                                                                                                                                                                                                                                                                                 |
